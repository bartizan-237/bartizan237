...
<div class="flex flx-wrap bg-white fixed bottom-0 w-full border-t border-gray-200 flex" style="max-width:500px; margin:0 auto; z-index: 10; height: 60px">
    <a id="nav_main" href="/home" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/home")) text-green-600 @else text-gray-800 @endif active:text-theme-01">
{{--    <a id="nav_main" onclick="toast('info', '준비 중 입니다')" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/home")) text-green-600 @else text-gray-800 @endif active:text-theme-01">--}}
...
{{--    </a>--}}
    ...
    <a id="" href="/bartizan" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2
            @if(str_contains($_SERVER["REQUEST_URI"], "/bartizan") AND !str_contains($_SERVER["REQUEST_URI"], "/user")) text-green-600 @else text-gray-800 @endif active:text-theme-01">
{{--    <a id="" onclick="toast('info', '준비 중 입니다')" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/bartizan")) text-green-600 @else text-gray-800 @endif active:text-theme-01">--}}
        <div class="text-center">
...
{{--    <a id="" href="/nation/main" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/nation")) text-green-600 @else text-gray-800 @endif active:text-theme-01">--}}
    <a id="" onclick="toast('info', '준비 중 입니다')" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 text-gray-400 active:text-theme-01">
        <div class="text-center">
...
{{--    </a>--}}
    ...
    @if(\Auth::user())
    <a id="" href="/user/my_page" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/user")) text-green-600 @else text-gray-800 @endif active:text-theme-01">
    @else
    <a onclick="toast('info', '준비 중 입니다')" class="nav_btn w-1/5  cursor-pointer flex flex-grow items-center justify-center p-2 @if(str_contains($_SERVER["REQUEST_URI"], "/user")) text-green-600 @else text-gray-400 @endif active:text-theme-01">
    @endif
        <div class="text-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mb-1" viewBox="0 0 20 20" fill="currentColor">
