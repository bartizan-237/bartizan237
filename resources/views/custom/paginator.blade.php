@if ($paginator->hasPages())
    <nav role="navigation" class="" style="max-width: 500px;">
        <div class="w-full px-2 flex items-center justify-center">
            <span class="relative z-0 inline-flex rounded-md">
                {{-- Previous Page Link --}}
                @if ($paginator->onFirstPage())
                    <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                        <span class="relative inline-flex items-center justify-center w-6 h-8 font-medium text-gray-300 bg-white cursor-default rounded-lg leading-5" aria-hidden="true">
                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span class="page" aria-current="page">
                                    <span class="relative inline-flex items-center justify-center w-6 h-8 mx-1 font-medium text-white bg-green-500 rounded-lg cursor-default leading-5 shadow-inner">{{ $page }}</span>
                                </span>
                            @else
                                <a href="{{ $url }}" class="relative inline-flex items-center justify-center w-6 h-8 mx-1 font-medium text-gray-700 bg-white rounded-lg leading-5 hover:text-gray-500 hover:bg-green-50 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-green-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
